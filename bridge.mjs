...
//  2. spawn 同目录的 claude-monitor-sl.exe 写 live/<pid>.json(异步,不阻塞)
...
const monitorDir = path.join(os.homedir(), '.claude-monitor');
...
try {
  const slhook = path.join(path.dirname(process.argv[1]), 'claude-monitor-sl.exe');
