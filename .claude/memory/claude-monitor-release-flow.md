---
name: claude-monitor-release-flow
description: claude-code-monitor 发新版本时必须同步改的版本号位置 + 发布命令
metadata: 
  node_type: memory
  type: project
...
发布 claude-code-monitor 新版本（如 v1.2.0）时，**必须先改代码里的版本号**，再打 tag + 创建 release。
...
**完整发布流程**（GitHub: `pie-tk/claude-code-monitor`，SSH 走 [[github-ssh-over-443]] 的 443）：
1. 改 `service/monitor_service.go` 的 `Version` 常量为新版本号
2. `go build -ldflags="-H windowsgui -s -w" -o claude-monitor.exe .`（前端先 `cd frontend && npm run build`）
3. git commit + push
4. `git tag v1.2.0 && git push origin v1.2.0`
5. `gh release create v1.2.0 claude-monitor.exe --title v1.2.0 --notes "..."`
...
`gh release upload v1.1.0 claude-monitor.exe --clobber`  # --clobber 覆盖同名 asset，不动 tag 与 git
